  ...
  try {
    ...
    recentRegistrations.forEach((reg: any) => {
      activities.push({
        id: reg._id,
        type: "registration",
        title: `${reg.user?.name || "Unknown"} registered for ${reg.tournament?.title || "Unknown Tournament"}`,
    ...
    recentTournaments.forEach((tournament: any) => {
      activities.push({
        ...
        type: "tournament",
        title: `New tournament "${tournament.title}" created by ${tournament.organizer?.name || "Unknown"}`,
